...
## X-Ray <Badge text="beta" type="warning"/># CloudLinux OS+ (BETA)
...
In the hint, there is a number of faults for each limit.
The values in the columns are the load-to-limit ratio. For the current implementation, the only sort by the load-to-limit ratio is available.
...
For the CloudLinux Plus clients we provide a monitoring solution that allows you to:

* View system metrics for all clients’ end servers
* View the LVE statistics per user for all clients’ end servers

<span class="notranslate">Centralized Monitoring</span> allows hosting administrators to monitor all their servers: how loaded the servers are, and which user creates the most load across the server.
